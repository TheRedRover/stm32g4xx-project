...
#define USART_CR1_FIELDS                                                                           \
    ((uint32_t) (USART_CR1_M | USART_CR1_PCE | USART_CR1_PS | USART_CR1_TE | USART_CR1_RE |        \
                 USART_CR1_OVER8 | USART_CR1_FIFOEN)) /*!< USART CR1 fields of parameters set by   \
                                                         USART_SetConfig API */
...
#define USART_CR2_FIELDS                                                                           \
    ((uint32_t) (USART_CR2_CPHA | USART_CR2_CPOL | USART_CR2_CLKEN | USART_CR2_LBCL |              \
                 ...
                 USART_CR2_DIS_NSS)) /*!< USART CR2 fields of parameters set by USART_SetConfig    \
                                        API */
...
/**
  ...
  * @note   The HAL_USART_RegisterCallback() may be called before HAL_USART_Init() in
 ...
 * @note   The HAL_USART_UnRegisterCallback() may be called before HAL_USART_Init() in
                                     ...
                                     uint16_t Size, uint32_t Timeout)
{
    ...
    if (husart->State == HAL_USART_STATE_READY)
    {
        ...
        {
            if (USART_WaitOnFlagUntilTimeout(husart, USART_FLAG_TXE, RESET, tickstart, Timeout) !=
                HAL_OK)
            {
                return HAL_TIMEOUT;
            }
        ...
        if (USART_WaitOnFlagUntilTimeout(husart, USART_FLAG_TC, RESET, tickstart, Timeout) !=
            HAL_OK)
        {
            return HAL_TIMEOUT;
        }
                                    ...
                                    uint32_t Timeout)
{
    ...
    if (husart->State == HAL_USART_STATE_READY)
    {
        ...
        while (husart->RxXferCount > 0U)
        {
            ...
            /* Wait for RXNE Flag */
            if (USART_WaitOnFlagUntilTimeout(husart, USART_FLAG_RXNE, RESET, tickstart, Timeout) !=
                HAL_OK)
            {
                return HAL_TIMEOUT;
            }
    ...
    if (husart->State == HAL_USART_STATE_READY)
    {
        ...
        if (husart->SlaveMode == USART_SLAVEMODE_DISABLE)
        {
            /* Send dummy data in order to generate the clock for the Slave to send the next data.
               ...
               When FIFO mode is enabled data must be transmitted until the RX FIFO reaches its
               threshold.
            */
            if ((husart->FifoMode == USART_FIFOMODE_ENABLE) && (Size >= husart->NbRxDataToProcess))
            {
...
{
    /* Disable TXEIE, TCIE, RXNE, RXFT, TXFT, PE and ERR (Frame error, noise error, overrun error)
     * interrupts */
    ...
    __HAL_USART_CLEAR_FLAG(husart,
                           USART_CLEAR_OREF | USART_CLEAR_NEF | USART_CLEAR_PEF | USART_CLEAR_FEF);
...
HAL_StatusTypeDef HAL_USART_Abort_IT(USART_HandleTypeDef *husart)
{
    ...
    /* If DMA Tx and/or DMA Rx Handles are associated to USART Handle, DMA Abort complete callbacks
...
void HAL_USART_IRQHandler(USART_HandleTypeDef *husart)
{
                               ...
                               ((cr1its & (USART_CR1_RXNEIE_RXFNEIE | USART_CR1_PEIE)) != 0U)))
    {
        ...
        if (husart->ErrorCode != HAL_USART_ERROR_NONE)
        {
            ...
            if ((HAL_IS_BIT_SET(husart->Instance->CR3, USART_CR3_DMAR)) || (errorcode != 0U))
            {
                ...
                if (HAL_IS_BIT_SET(husart->Instance->CR3, USART_CR3_DMAR))
                {
                    ...
                    if (husart->hdmarx != NULL)
                    {
                        ...
                        if (HAL_DMA_Abort_IT(husart->hdmarx) != HAL_OK)
                        {
                            /* Call Directly husart->hdmarx->XferAbortCallback function in case of
                             * error */
...
static HAL_StatusTypeDef USART_SetConfig(USART_HandleTypeDef *husart)
{
     ...
     * set LBCL bit according to husart->Init.CLKLastBit value (used in USART Synchronous SPI master
     * mode only) set STOP[13:12] bits according to husart->Init.StopBits value */
                ...
                if (husart->RxXferCount == 0U)
                {
                    ...
                    else
                    {
                        /* Nothing to do */
                         ...
                         (husart->SlaveMode == USART_SLAVEMODE_DISABLE))
                {
                    /* Send dummy byte in order to generate the clock for the Slave to Send the next
                     * data */
                    husart->Instance->TDR = (USART_DUMMY_DATA & (uint16_t) 0x00FF);
                }
                else
