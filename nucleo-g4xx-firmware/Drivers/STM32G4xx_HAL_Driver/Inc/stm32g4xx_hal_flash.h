...
typedef struct
{
    ...
    uint32_t Page;      /*!< Initial Flash page to erase when page erase is disabled.
                           ...
                           bank - 1)      (eg : 127 for 512KB dual bank) */
...
typedef struct
{
    ...
    uint32_t SecSize; /*!< Size of securable memory area to be programmed (used for OPTIONBYTE_SEC),
                           in number of pages. Securable memory area is starting from first page of
                         the bank. Only one securable memory could be programmed at the same time.
                           This parameter must be a value between 0 and (max number of pages in the
                         bank - 1) */
...
#define FLASH_TYPEPROGRAM_FAST                                                                     \
    0x01U /*!< Fast program a 32 row double-word (64-bit) at a specified address.                  \
               And another 32 row double-word (64-bit) will be programmed */
#define FLASH_TYPEPROGRAM_FAST_AND_LAST                                                            \
    0x02U /*!< Fast program a 32 row double-word (64-bit) at a specified address.                  \
...
#define OB_USER_DBANK                                                                              \
    0x00000100U /*!< Single bank with 128-bits data or two banks with 64-bits data */
#endif
...
#define OB_PB4_PUPEN_DISABLE                                                                       \
    0x00000000U /*!< USB power delivery dead-battery enabled/ TDI pull-up deactivated */
#define OB_PB4_PUPEN_ENABLE                                                                        \
    FLASH_OPTR_PB4_PUPEN /*!< USB power delivery dead-battery disabled/ TDI pull-up activated */
/**
 * @}
 */
#endif
...
#define OB_SRAM_PARITY_DISABLE                                                                     \
    FLASH_OPTR_SRAM_PE /*!< SRAM parity check disable (first 32kB of SRAM1 + CCM SRAM) */
/**
 * @}
 */
...
#define FLASH_FLAG_ECCC2                                                                           \
    FLASH_ECCR_ECCC2 /*!< FLASH ECC correction in 64 MSB bits (mode 128 bits only) */
#define FLASH_FLAG_ECCD2                                                                           \
    FLASH_ECCR_ECCD2 /*!< FLASH ECC detection in 64 MSB bits (mode 128 bits only) */
#endif
 ...
 *     @arg FLASH_FLAG_ECCC2(*): FLASH one ECC error has been detected and corrected in 64 MSB bits
 * (mode 128 bits only)
 *     @arg FLASH_FLAG_ECCD2(*): FLASH two ECC errors have been detected in 64 MSB bits (mode 128
 * bits only)
 * @note  (*) availability depends on devices
