 ...
 */
int32_t BSP_GetVersion(void) { return (int32_t) STM32G4XX_NUCLEO_BSP_VERSION; }

/**
 * @brief  This method returns the board name
 * @retval pointer to the board name string
 */
const uint8_t *BSP_GetBoardName(void) { return (const uint8_t *) STM32G4XX_NUCLEO_BSP_BOARD_NAME; }

/**
 * @brief  This method returns the board ID
 * @retval pointer to the board ID string
 */
const uint8_t *BSP_GetBoardID(void) { return (const uint8_t *) STM32G4XX_NUCLEO_BSP_BOARD_ID; }
...
int32_t BSP_COM_RegisterDefaultMspCallbacks(COM_TypeDef COM)
{
    ...
    else
    {
        ...
        if (HAL_UART_RegisterCallback(&hcom_uart[COM], HAL_UART_MSPINIT_CB_ID, COM1_MspInit) !=
            HAL_OK)
        {
