 ...
 * @{
 */
static uint32_t CRC_Handle_8(CRC_HandleTypeDef *hcrc, uint8_t const pBuffer[],
                             uint32_t BufferLength);
static uint32_t CRC_Handle_16(CRC_HandleTypeDef *hcrc, uint16_t const pBuffer[],
                              uint32_t BufferLength);
/**
 * @}
 */
...
HAL_StatusTypeDef HAL_CRC_Init(CRC_HandleTypeDef *hcrc)
{
    ...
    else
    {
        ...
        if (HAL_CRCEx_Polynomial_Set(hcrc, hcrc->Init.GeneratingPolynomial, hcrc->Init.CRCLength) !=
            HAL_OK)
        {
            return HAL_ERROR;
        }
                            ...
                            uint32_t BufferLength)
{
    ...
    case CRC_INPUTDATA_FORMAT_HALFWORDS:
        temp = CRC_Handle_16(hcrc, (uint16_t const *) (void const *) pBuffer,
